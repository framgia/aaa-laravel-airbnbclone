    ...
    public function search(Request $request)
    {
        if (!empty($queries = $request->query())) {
            ...
            if ($request->has('address')) {
                ...
                $rooms = Room::active()->near($coordinates, 5);
            }
                ...
                if (!is_null($queries['start_date']) && !is_null($queries['end_date'])) {
                    ...
                    foreach($rooms as $key => $room) {
                        $unavailable = $room->reservations()
                                            ->whereRaw("(? <= start_date AND start_date <= ?)", [$queries['start_date'], $queries['end_date']])
                                            ->orWhereRaw("(? <= end_date AND end_date <= ?)", [$queries['start_date'], $queries['end_date']])
                                            ->orWhereRaw("(start_date < ? AND ? < end_date)", [$queries['start_date'], $queries['end_date']])
                                            ->limit(1)->get();
