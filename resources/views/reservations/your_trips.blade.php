                            ...
                            <div class="col-md-8">
                                ...
                                <span class="float-right text-center">
                                    @if ($trip->room->user->socialAccount)
                                        <img src="{{ $trip->room->user->socialAccount->image }}" class="rounded-circle" width="48px">
                                    @else
                                        <img src="{{ $trip->room->user->gravatar(48) }}" class="rounded-circle">
                                    @endif
                                    <a href="{{ route('users.show', $trip->room->user) }}">{{ $trip->room->user->name }}</a>
                                </span>
                            </div>
                        </div>
                        <hr/>
