            ...
            <div class="row">
                ...
                <div class="col-md-4 text-center">
                    @if ($room->user->socialAccount)
                        <img src="{{ $room->user->socialAccount->image }}" class="rounded-circle">
                    @else
                    ...
                    @foreach ($room->nearbys(10) as $nearbyRoom)
                        <div class="col-md-4">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <a href="{{ route('rooms.show', $nearbyRoom) }}">{{ $room->listing_name }}</a>
                                    <br/>
            ...
            success: function(data) {
                $.each(data, function(key, value){
                    for(var date = new Date(value.start_date); date <= new Date(value.end_date); date.setDate(date.getDate() + 1)) {
                        unavailableDates.push($.datepicker.formatDate('yy-m-dd', date));
                    }
                });
